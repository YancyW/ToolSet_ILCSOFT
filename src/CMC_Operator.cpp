...
MCParticle* ToolSet::CMC::NewParticle(TLorentzVector &p, int pdg, float charge, MCParticle* test){
	MCParticleImpl* tmp= new MCParticleImpl;
	double Pnew[3] = {p.Px(),p.Py(),p.Pz()};
    tmp->setMomentum(Pnew);
	double mass2= std::pow(p.E(),2)-std::pow(p.Px(),2)-std::pow(p.Py(),2)-std::pow(p.Pz(),2);
...
MCParticle&  ToolSet::operator -(const MCParticle& MC1, const MCParticle& MC2) {
	...
	TVector3  p2=MC2.getMomentum();
	TVector3  p0=p1-p2;
	double energy=std::abs((MC1.getEnergy()-MC2.getEnergy()));
	double mass2= std::pow(energy,2)-p0.Mag2();
	double     mass;
	if(mass2>=0){
		mass= std::sqrt(mass2);
...
TLorentzVector  ToolSet::CMC::Minus_By_Lorentz(const MCParticle* MC1, const MCParticle* MC2) {
